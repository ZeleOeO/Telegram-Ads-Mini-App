...
pub async fn initiate_payment(
    ...
    let wallet = if let Some(existing) = existing_wallet {
        services::escrow_mock::EscrowWallet {
            address: existing.address,
            balance: existing.balance_ton.to_string().parse().unwrap_or(0.0),
        }
    } else {
        services::escrow_mock::generate_escrow_wallet(&state.db, deal_id).await?
    };
...
pub async fn verify_payment(
    ...
    let verified =
        services::escrow_mock::verify_payment(&state.db, &wallet.address, expected_amount).await?;
...
pub async fn release_funds(
    ...
    let tx_hash = services::escrow_mock::release_funds(&state.db, deal_id, &to_address).await?;
