    ...
    const handleAddPricing = async (e: React.FormEvent) => {
        ...
        try {
            await api.post(`/channels/${channel.id}/ad-formats`, {
                ...
                format_description: `Standard ${format.toLowerCase()} kept for ${duration}`,
            });
            setPrice('');
            loadFormats();
            onSuccess();
        ...
        <div className="fixed inset-0 z-[110] flex items-end sm:items-center justify-center sm:p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="w-full sm:max-w-md glass rounded-t-2xl sm:rounded-2xl relative max-h-[90vh] overflow-y-auto font-sans">
                <div className="p-4 border-b border-white/10 flex justify-between items-center sticky top-0 bg-[#0b0e14]/80 backdrop-blur-md z-10">
                    <h3 className="text-sm font-bold uppercase tracking-wider text-white/60">Channel Settings</h3>
                ...
                <div className="flex p-2 gap-1">
                    <button
                        ...
                        className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${activeTab === 'pricing' ? 'bg-white/10 text-white' : 'text-white/30'}`}
                    >
                        <DollarSign size={14} className="inline mr-1" /> Pricing
                    </button>
                    <button
                        onClick={() => setActiveTab('stats')}
                        className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${activeTab === 'stats' ? 'bg-white/10 text-white' : 'text-white/30'}`}
                    >
                        <BarChart2 size={14} className="inline mr-1" /> General
                    </button>
                    <button
                        onClick={() => setActiveTab('team')}
                        className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${activeTab === 'team' ? 'bg-white/10 text-white' : 'text-white/30'}`}
                    >
                ...
                <div className="p-6 pt-2 pb-24">
                    {activeTab === 'pricing' && (
                        <div className="space-y-6">
                                ...
                                <div className="p-4 space-y-4">
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <div className="space-y-1.5">
                                            <label className="text-[10px] text-white/40 uppercase font-bold ml-1">Type</label>
                                            <select value={format} onChange={e => setFormat(e.target.value)} className="w-full h-11 bg-black/40 border border-white/10 rounded-xl px-3 text-xs text-white focus:outline-none focus:border-blue-500/50 appearance-none">
                                        ...
                                        <div className="space-y-1.5">
                                            <label className="text-[10px] text-white/40 uppercase font-bold ml-1">Duration</label>
                                            <select value={duration} onChange={e => setDuration(e.target.value)} className="w-full h-11 bg-black/40 border border-white/10 rounded-xl px-3 text-xs text-white focus:outline-none focus:border-blue-500/50 appearance-none">
                                    ...
                                    <div className="space-y-1.5">
                                        <label className="text-[10px] text-white/40 uppercase font-bold ml-1">Price (TON)</label>
                                        <div className="relative">
                                            <input
                                                ...
                                                placeholder="0.0"
                                                className="w-full h-12 bg-black/40 border border-white/10 rounded-xl pl-4 pr-12 text-sm text-white focus:outline-none focus:border-blue-500/50 transition-all"
                                                required
                                            />
                            ...
                            <div className="space-y-3">
                                <h4 className="text-[10px] font-bold text-white/40 uppercase tracking-widest ml-1">Active Ad Formats</h4>
                                <div className="space-y-2">
                                    {formats.map(f => (
                                        <div key={f.id} className="group p-4 bg-white/5 rounded-2xl border border-white/5 flex items-center justify-between hover:border-white/10 transition-all">
                                            <div className="space-y-1">
                                                ...
                                                <div className="text-[10px] text-white/30 uppercase font-medium">Standard Placement</div>
                                            </div>
                                    ...
                                    {formats.length === 0 && (
                                        <div className="text-center py-10 bg-white/5 rounded-2xl border border-dashed border-white/10">
                                            <p className="text-xs text-white/20 italic">No formats listed yet</p>
                                        </div>
                                    )}
                                </div>
                            </div>
                    ...
                    {activeTab === 'stats' && (
                        <div className="space-y-6">
                            ...
                            <form onSubmit={handleUpdateStats} className="space-y-5">
                                <div className="space-y-1.5">
                                    <label className="text-[10px] text-white/40 uppercase font-bold ml-1">Channel Category</label>
                                    <select
                                        value={category}
                                ...
                                <div className="space-y-1.5">
                                    <label className="text-[10px] text-white/40 uppercase font-bold ml-1">Primary Language</label>
                                    <select
                                        value={language}
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div className="p-4 bg-white/5 rounded-2xl border border-white/5">
                                    <div className="text-[10px] text-white/40 uppercase font-bold mb-1">Current Reach</div>
                                    <div className="text-lg font-bold">{Number(channel.reach || 0).toLocaleString()}</div>
                                </div>
                                <div className="p-4 bg-white/5 rounded-2xl border border-white/5">
                                    <div className="text-[10px] text-white/40 uppercase font-bold mb-1">Premium %</div>
                    ...
                    {activeTab === 'team' && (
                        <div className="space-y-6">
                            ...
                            <div className="space-y-3">
                                <div className="space-y-1.5">
                                    <label className="text-[10px] text-white/40 uppercase font-bold ml-1">Invite by Username or ID</label>
